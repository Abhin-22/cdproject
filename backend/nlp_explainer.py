...
=========================
NLP-powered error explanation engine.

Priority chain
──────────────
1. Local Ollama          — if installed & running on this machine
2. HF Inference API      — FREE cloud API, just needs HF_TOKEN in .env
                           Works on ANY machine with internet. No install.
3. Anthropic Claude API  — cloud fallback if ANTHROPIC_API_KEY is set
4. Rule-based NLP        — always works, zero network, zero tokens

Transport story
───────────────
Copy the project folder + .env (with HF_TOKEN filled in) to any machine.
Run: pip install -r requirements.txt  then: python server.py
AI explanations work immediately — no Ollama, no GPU, no model downloads.
"""
...
def _build_prompt(errors: List[dict], source: str, lang: str, level: str) -> str:
    ...
    return (
        ...
        f"Full source:\n```{lang}\n{source[:2000]}\n```\n\n"
        f"{level_prompt}\n\n"
        f"Return ONLY a JSON array, one object per error, in order:\n"
        f'[\n  {{"title":"4-6 word title","explanation":"Plain English explanation","fix":"Specific actionable fix","fixed_line":"Corrected version of that source line"}}\n]\n\n'
        f"Return ONLY valid JSON. No markdown fences, no preamble, no commentary."
    )
...
# ── 1. Local Ollama (optional, only if installed) ─────────────────────────────
...
# Free cloud API — just needs HF_TOKEN in .env
# Works on every machine with internet — nothing to install, no GPU needed
...
def hf_status() -> dict:
    """Probe HF Inference API — verify token works without running a full query."""
    if not cfg.hf_ready():
    ...
    try:
        ...
        # 503 = model is loading (token still valid)
        return {"ready": True, "model": cfg.HF_MODEL,
                "preview": cfg.hf_token_preview(), "error": None}
...
def _explain_hf(errors, source, lang, level, model=None):
    """Call HF Inference API. Portable — just needs HF_TOKEN."""
    if not cfg.hf_ready():
...
def explain_ai(errors: List[dict], source: str,
               lang: str = "cpp", level: str = "beginner",
               prefer_ollama: bool = True,
               ollama_model:  str | None = None,
               hf_model:      str | None = None) -> tuple[List[dict], bool]:
    """
    Priority chain:
      1. Local Ollama   — if prefer_ollama=True and Ollama is reachable
      2. HF Inference   — portable, free cloud AI, needs HF_TOKEN in .env
      3. Anthropic Claude — needs ANTHROPIC_API_KEY in .env
      4. Rule-based NLP — always works offline
    """
    if prefer_ollama:
        try:
